#' Summarize a linear model with two explanatory variables
#'
...
#' @export
#' @examples
#' two_explan_mlm_summary(snapper_full, 'Total_length_cm', 'Sex_dbl', 'Age_dbl')
two_explan_mlm_summary = function (df_name, response_variable, explanatory_variable1, explanatory_variable2) {
  response = df_name[,response_variable]
  explanatory1 = df_name[,explanatory_variable1]
  explanatory2 = df_name[,explanatory_variable2]
  stats::lm(response~explanatory1 + explanatory2) %>% summary()
}
